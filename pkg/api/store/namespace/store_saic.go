...
type SAICStore struct {
	types.Store
}

func (p *SAICStore) Create(apiContext *types.APIContext, schema *types.Schema, data map[string]interface{}) (map[string]interface{}, error) {
...
func (p *SAICStore) validateResourceQuota(apiContext *types.APIContext, schema *types.Schema, data map[string]interface{}, id string, update bool) error {
	...
	projectQuotaLimitLen := len(projectQuotaLimitMap)

	if _, ok := projectQuotaLimitMap["servicesAllocatedPorts"]; ok {
		projectQuotaLimitLen = projectQuotaLimitLen - 1
	}

	if len(nsQuotaLimitMap) != projectQuotaLimitLen {
