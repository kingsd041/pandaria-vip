...
func (ch *clusterHandler) ensureAppProjectName(clusterID, appTargetNamespace string) (string, error) {
	appDeployProjectID, err := gpu.GetSystemProjectID(ch.app.cattleProjectClient)
	if err != nil {
		return "", err
	}

	appProjectName, err := gpu.EnsureAppProjectName(ch.app.agentNamespaceClient, appDeployProjectID, clusterID, appTargetNamespace)
	if err != nil {
		return "", err
	}
...
func (ch *clusterHandler) deployApp(appName, appTargetNamespace string, appProjectName string, cluster *mgmtv3.Cluster) error {
	...
	_, err = gpu.DeployApp(ch.app.cattleAppClient, appDeployProjectID, app, false)
	if err != nil {
		return err
	}
